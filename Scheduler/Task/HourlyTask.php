...
/**
 * Class to handle Hourly scheduler.
 * @package    Scheduler
 * @subpackage Task
 * @author     Deni Firdaus Waruwu <deni.firdaus.w@gmail.com>
 * @copyright  2015-2016 Deni Firdaus Waruwu
 */
class HourlyTask extends \Scheduler\AbstractTask
{

    /**
     * Function construct of class \HourlyTask.
     *
    ...
    public function __construct($name, $action)
    {
        ...
        $this->setStartDateTime($dateTime->add(new \DateInterval('PT1H')));
    }
    ...
    {
        $listError = $this->validateSchtasksToRegisterHourlyTask();
    ...
    /**
     * Function to validate sch syntax to register hourly task.
     *
     * @return array
     */
    private function validateSchtasksToRegisterHourlyTask()
    {
        $errorMessage = [];
        ...
        if (empty(trim($this->getModifier())) === true or $this->getModifier() === null) {
            ...
            if (is_numeric($this->getModifier()) === true) {
                ...
                if ($intModifier < 1 or $intModifier > 24) {
                    $errorMessage[] = "Value of Modifier is not valid. It's must be numeric with value 1 to 24.";
                }
            } else {
                $errorMessage[] = "Value of Modifier is not valid. It's must be numeric with value 1 to 24.";
            }
    ...
    /**
     * Function to generate query syntax for registering Hourly task.
     *
     * @return string
     */
    private function generateRegisterQuery()
    {
        $query = 'SCHTASKS /CREATE /SC HOURLY';
    ...
    /**
     * Function to generate query syntax for updating Hourly task.
     *
     * @return string
     */
    private function generateUpdateQuery()
    {
        $query = 'SCHTASKS /CHANGE /SC HOURLY';
        $query .= ' ' . $this->getSchSyntaxForModifier();
        $query .= ' ' . $this->getSchSyntaxForName();
        $query .= ' ' . $this->getSchSyntaxForAction();
        $query .= ' ' . $this->getSchSyntaxForStartDate();
